...
namespace Game.LoopHero
{
    public static partial class ItemCardEffectExecutes
    {
        ...
        /// </summary>
        [EffectForItemCard(ItemEnum.重组升级)]
        public static bool Exe重组升级(ItemEnum cardId, Collider2D collider2D)
        {
            GameLogger.Log.Debug($"Exe重组升级 {cardId} {collider2D}");
            Assert.IsTrue(cardId == ItemEnum.重组升级);
            ...
            {
                pokemon.data.AddPermanentHealth(5);
                return true;
            }

            pokemon.data.AddTempHealthPercent(0.5f);
            return true;
        }
    }
}