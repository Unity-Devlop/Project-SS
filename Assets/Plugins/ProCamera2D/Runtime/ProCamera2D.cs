...
namespace Com.LuisPedroFonseca.ProCamera2D
{
    ...
    public class ProCamera2D : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        public void UpdateScreenSize(float newSize, float duration = 0f, EaseType easeType = EaseType.EaseInOut, Action< float > finish = null )
        {
            if (!enabled)
                return;
            ...
            if (duration > 0)
                _updateScreenSizeCoroutine = StartCoroutine(UpdateScreenSizeRoutine(newSize, duration, easeType, finish));
            else
            {
                SetScreenSize(newSize);
                finish?.Invoke( newSize );
            }
        }
        ...
        /// <param name="easeType">The easing method to apply. Only used when the duration is bigger than 0</param>
        public void Zoom(float targetSize, float duration = 0f, EaseType easeType = EaseType.EaseInOut, Action< float > finish = null )
        {
            UpdateScreenSize(targetSize,duration, easeType, finish );
        }
        ...
        public void DollyZoom(float targetFOV, float duration = 1f, EaseType easeType = EaseType.EaseInOut, Action<float> finish = null)
        {
            if (!enabled)
                return;
            ...
            if (duration <= 0)
            {
                ...
                finish?.Invoke(GameCamera.fieldOfView);
            }
            else
            {
                StartCoroutine(DollyZoomRoutine(targetFOV, duration, easeType, finish));
            }
        }

        public void StopDollyZoomCoroutine()
        {
            if (_dollyZoomRoutine != null)
            {
                StopCoroutine(_dollyZoomRoutine);
        ...
        IEnumerator UpdateScreenSizeRoutine(float finalSize, float duration, EaseType easeType, Action<float> finish)
        {
        ...
        IEnumerator DollyZoomRoutine(float finalFOV, float duration, EaseType easeType, Action< float > finish = null )
        {
        ...
        public float GetCameraSizeForFOV(float fov)
        {
            return ( 2f * Mathf.Abs(Vector3D(_transform.localPosition) * Mathf.Tan( fov * 0.5f * Mathf.Deg2Rad ) ) ) / 2f ;
        }
